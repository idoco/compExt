...
function Rule(data) {
  ...
  if (data) {
	...
	this.getElement('match-substring').checked = data.match_substring;
  }
  ...
  this.getElement('match-substring').onchange = storeRules;
	...
	chrome.storage.local.get("rules",
		function(result){
			...
			try {				
				...
				console.log(e+" - Could not reload initial rule set, please reload the extension");			
			}
		}
	);  
}
		...
		.map(
			function(node) {
				return {match_word: node.rule.getElement('match-word').value,
						...
						match_substring: node.rule.getElement('match-substring').checked};
			}
		)
	});
}
